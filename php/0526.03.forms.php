            ...
            <div class="col-lg-6">
                <form name='form1' onsubmit="senddata(); return false;">
                    <div class="mb-3">
                        ...
                        <select class="form-select form-select-lg mb-3" aria-label=".form-select-lg example" name="hobby1">
                            ...
                            <?php foreach ($hobbies as $k => $v) : ?>
                                <option value="<?= $k ?>" <?= $data['postdata']['hobby1'] == $k ? 'selected' : '' ?>>
                                    <!-- 如果key值有對應到$data['postdata']['hobby1']的值，會顯示為selected -->
                                    <?= $v ?>
                                </option>
                            <?php endforeach ?>
                        </select>
                    ...
                    <div class="mb-3">
                        ...
                        <?php foreach ($hobbies as $k => $v) : ?>
                            <div class="form-check">
                                <input class="form-check-input" type="checkbox" name="hobby3[]" id="hobby3-<?= $k ?>" value="<?= $k ?>" <?= in_array($k, $data['postdata']['hobby3']) ? 'checked' : '' ?>>
                                <!-- in_array($k, $data['postdata']['hobby3']) -->
                                <!-- k值在array裡有沒有找到 -->
